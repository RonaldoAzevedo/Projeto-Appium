...
public class MobileBank<T> extends Mobile {
	...
	public PgoSimulacaoConsignado consig;
	...
	protected void aguardarLoadSistema() {
		...
		aguardaElemento(ExpectedConditionsCustom.stalenessOf(login.ICONE_LOADER));
	}
	...
	public String pegarTextoElementoPorAtributo(MobileElement object, String atributo) {
		return object.getAttribute('"'+atributo+'"').toLowerCase();
	}
...
//		click(menu.BOTAO_VOLTAR_SUPERIOR);
	...
	public T preencherSenha(String senha) {
		swipe(ScrollDirection.DOWN, consig.CAMPO_SENHA, menu.TELA_PADRAO);
		preencheCampo(consig.CAMPO_SENHA, senha);
		return (T) this;
	}
	...
	protected void preencherComboFiscalizacao(WebElement elemento, Object opcao) {
		click(elemento);
		aguardaElemento2(elemento.findElement(By.xpath("//android.widget.ListView")));
		selecionarCombo(elemento.findElement(By.xpath("//android.widget.ListView")),
				new ByXPath("//android.view.View[@text="+"'"+ opcao +"'"+"]"));
	}
	
	protected void preencherComboFiscalizacaoComContenDesc(WebElement elemento, Object opcao) {
		click(elemento);
		aguardaElementoSerVisivel(elemento.findElement(By.xpath("//android.widget.ListView")));
		selecionarCombo(elemento.findElement(By.xpath("//android.widget.ListView")),
	...
	public String gerarStringAleatoria() {
		...
		Random random = new Random();
		String armazenaApelido = "";
		int index = -1;
		for( int i = 0; i < 9; i++ ) {
		   index = random.nextInt( letras.length() );
		   armazenaApelido += letras.substring( index, index + 1 );
		}
		Log.info("Random string: "+ armazenaApelido );
		return armazenaApelido;
	}
	
	public void aguardaElementoSerVisivel(WebElement el) {
        try {
        	WebDriverWait mWait = new WebDriverWait(ActionUi.getDriver(), 45000);
